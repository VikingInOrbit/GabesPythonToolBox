...
class ConfigManager:
    ...
    def loadConfig(
        self, configFilePath: str = None
    ):  # TODO how do i deal with / and \ in the file name?
        """
        ...
        if not os.path.exists(configFilePath):
            ...
            raise FileNotFoundError(
                f"Configuration file {configFilePath} not found."
            )  # TODO dues not print right
        ...
        try:
            ...
            for k in keys[:-1]:
                if "[" in k and k.endswith("]"):
                    ...
                    array_key = (
                        int(array_key) if str(array_key).isdigit() else array_key
                    )
            ...
            if "[" in final_key and final_key.endswith("]"):
                ...
                final_key_idx = (
                    int(final_key) if str(final_key).isdigit() else final_key
                )
    ...
    def add(self, data):
        ...
        if isinstance(data, dict):
            ...
            self.config.update(
                data
            )  # Append the new data to the existing configuration
