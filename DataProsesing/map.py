...
def Map(
    input: float, minInput: float, maxInput: float, minOutput: float, maxOutput: float
):
    ...
    Debug.log(
        f"input value: {input}, minInput: {minInput}, maxInput: {maxInput}, minOutput: {minOutput}, maxOutput: {maxOutput}",
        "Info",
        group="LIB",
    )
    ...
    if not all(
        ...
        for arg in (input, minInput, maxInput, minOutput, maxOutput)
    ):
        raise TypeError("All arguments must be numbers (int or float).")

    output = (input - minInput) / (maxInput - minInput) * (
        maxOutput - minOutput
    ) + minOutput
...
def Clamp(
    val: float, minOutput: float = None, maxOutput: float = None
) -> float:  # shood be able to get just max or just min
    ...
    Debug.log(
        f"input value: {val}, minOutput: {minOutput}, maxOutput: {maxOutput}",
        "Info",
        group="LIB",
    )
