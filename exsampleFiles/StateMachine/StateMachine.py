...
def main():
    ...
    # Log current state
    Debug.log(
        f"Current State: {SM.GetCurrentStateName()}", LogType.Info, LogGroup.Showcase
    )
    ...
    Debug.log(
        f"Default state: {SM.GetDefaultStateName()}, Current state: {SM.GetCurrentStateName()}",
        LogType.Info,
        LogGroup.Showcase,
    )
    SM.RequestDefaultState()
    Debug.log(
        f"New Current state: {SM.GetCurrentStateName()}",
        LogType.Info,
        LogGroup.Showcase,
    )
    ...
    for target in ["GoHomeState", "ExploreState", "SleepState", "NoneTestState"]:
        ...
        Debug.log(
            f"Target: {target}, Success: {success}", LogType.Info, LogGroup.Showcase
        )
    ...
    Debug.log(
        f"Default State: {SM.GetDefaultStateName()}", LogType.Info, LogGroup.Showcase
    )

    # --- Return to default state again ---
    Debug.log(
        "Returning to default state with Teleport", LogType.Info, LogGroup.Showcase
    )
    SM.RequestDefaultState(Teleport=True)
    Debug.log(
        f"Current State: {SM.GetCurrentStateName()}", LogType.Info, LogGroup.Showcase
