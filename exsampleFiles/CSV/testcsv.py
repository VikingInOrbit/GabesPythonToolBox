...
def main(
    ...
    data = read_data(
        file_path,
        seperator_symbol=seperator_symbol,
    ...
    data = helper_column(
        ...
        "Oppslutning prosentvis ajusted",
        lambda x: (float(x) / count(data, "Fylkenavn")),
    )
    ...
    data = merge_data_by(
        data,
        "Partinavn",
        ["Antall stemmer totalt", "Antall mandater", "Oppslutning prosentvis ajusted"],
    )
    ...
    data = rebase_data(
        data,
        ["Partinavn", "Antall stemmer totalt", "Antall mandater"],
        get_column_widths(data),
    )
