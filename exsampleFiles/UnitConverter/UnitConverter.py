...
Debug.log(f"{length_in_meters} meters is equal to {length_in_km} kilometers.", "Info", group="Showcase")

# Example 2: Mass conversion (grams -> kilograms)
mass_in_grams = 500
mass_in_kg = converter.convert(mass_in_grams, 'g', 'kg', 'mass')
Debug.log(f"{mass_in_grams} grams is equal to {mass_in_kg} kilograms.", "Info", group="Showcase")

# Example 3: Time conversion (minutes -> seconds)
time_in_minutes = 5
time_in_seconds = converter.convert(time_in_minutes, 'min', 's', 'time')
Debug.log(f"{time_in_minutes} minutes is equal to {time_in_seconds} seconds.", "Info", group="Showcase")

# Example 4: Temperature conversion (Celsius -> Fahrenheit)
temp_in_celsius = 25
try:
    temp_in_fahrenheit = converter.convert(temp_in_celsius, 'C', 'F', 'temperature')
    Debug.log(f"{temp_in_celsius}°C is equal to {temp_in_fahrenheit}°F.", "Info", group="Showcase")
except Exception as e:
    Debug.log(f"Temperature conversion not implemented yet: {e}", "Warning", group="Showcase")
